/**
 * This package contains Photran's error parsers for various Fortran compilers.
 * <p>
 * Error parsers for the Intel and IBM (XLF) compilers are in separate plug-ins since those
 * compilers have their own features (which also contribute Managed Build support).
 * <p>
 * These are all contributed via plugin.xml.
 */
package org.eclipse.photran.internal.cdtinterface.errorparsers;